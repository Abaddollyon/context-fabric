...
You should see a JSON response listing all 16 MCP tools. For persistent storage (so memories survive container restarts), use a named volume:
...
Send a `tools/list` request to confirm all 16 MCP tools are available:
...
- **[Tools Reference](tools-reference.md)** — All 16 MCP tools with full parameter docs
