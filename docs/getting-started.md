...
You should see a JSON response listing all 25 MCP tools. For persistent storage (so memories survive container restarts), use a named volume:
...
Send a `tools/list` request to confirm all 25 MCP tools are available:
...
- **[Tools Reference](tools-reference.md)** — All 25 MCP tools with full parameter docs
- **[Skills](skills.md)** — Procedural memory: create and invoke reusable instruction blocks
- **[MCP Primitives](mcp-primitives.md)** — Resources (`memory://...`) and Prompts (`cf-*` slash-commands)
