...
describe('fuseRRF', () => {
  it('should rank items appearing in both lists higher than items in one list', async () => {
    const listA = [mem('a', 0.9), mem('b', 0.8), mem('c', 0.7)];
    const listB = [mem('b', 0.85), mem('d', 0.6), mem('a', 0.5)];

    const fused = await ContextEngine.fuseRRF(listA, listB, 10);
  ...
  it('should handle disjoint lists', async () => {
  ...
  it('should handle empty list A', async () => {
    const listB = [mem('x', 0.5), mem('y', 0.4)];
    const fused = await ContextEngine.fuseRRF([], listB, 10);
    expect(fused).toHaveLength(2);
    expect(fused[0].id).toBe('x');
  });

  it('should handle empty list B', async () => {
    const listA = [mem('x', 0.5)];
    const fused = await ContextEngine.fuseRRF(listA, [], 10);
    expect(fused).toHaveLength(1);
    expect(fused[0].id).toBe('x');
  });

  it('should handle both lists empty', async () => {
    const fused = await ContextEngine.fuseRRF([], [], 10);
  ...
  it('should respect the limit parameter', async () => {
    ...
    const fused = await ContextEngine.fuseRRF(listA, listB, 2);
    expect(fused).toHaveLength(2);
  });

  it('should deduplicate by ID and keep higher similarity version', async () => {
  ...
  it('should normalize top result to 1.0', async () => {
  ...
  it('should normalize relative scores preserving order', async () => {
    const listA = [mem('a', 0.9), mem('b', 0.8)];
  ...
  it('should use custom k parameter', async () => {
    ...
    const result = await ContextEngine.fuseRRF(listA, listB, 10, 60);
